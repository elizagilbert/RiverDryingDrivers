...
#plotting
conf_marss1 <- fitted(MD_3states_dry_uncon_BFGS, type = "ytT", interval = "confidence")
pred_marss1 <- fitted(MD_3states_dry_uncon_BFGS, type = "ytT", interval = "prediction")
df1 <- cbind(conf_marss1, pred_marss1[, c(".lwr", ".upr")]) %>% 
  rename(Reach = 1)

conf_marss2 <- fitted(MD_2states_div_uncon_BFGS, type = "ytT", interval = "confidence")
pred_marss2 <- fitted(MD_2states_div_uncon_BFGS, type = "ytT", interval = "prediction")
df2 <- cbind(conf_marss2, pred_marss2[, c(".lwr", ".upr")]) %>% 
  rename(Reach = 1)


ggplot(df1, aes(x = t, y = .fitted)) +
...
pl_drying <- ggplot(df1, aes(x = t, y = y))+
  ...
  geom_point(aes(x = t, y = .fitted), color = "red")+
  facet_grid(vars(Reach))+
...
pl_diverison <- ggplot(df2, aes(x = t, y = y))+
  geom_line(linewidth = 2)+
  geom_point(aes(x = t, y = .fitted), color = "purple")+
  facet_grid(vars(Reach))+
  ggtitle("2 diversion reaches")+
  theme_classic()


grid.arrange(pl_drying, pl_diverison)
...
#R squared
Sum_MD_3_resids <- sum((df1$y - df1$.fitted)^2)
TotalSUm_MD_3_resids <- sum((df1$y - mean(df1$y))^2)
Rsquared_MD_3 <- 1- (Sum_MD_3_resids/TotalSUm_MD_3_resids)

Sum_MD_2_resids <- sum((df2$y - df2$.fitted)^2)
TotalSUm_MD_2_resids <- sum((df2$y - mean(df2$y))^2)
Rsquared_MD_2 <- 1- (Sum_MD_2_resids/TotalSUm_MD_2_resids)
