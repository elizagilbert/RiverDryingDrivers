...
dat2 <- dat %>% filter(Date >= "2010-01-01") %>% 
  ...
  mutate(KMTenthDry = (RMTenthDry / 0.621371))
...
ann_cum_plt <- ggplot(Ann_cum_dry, aes(x = Date, y = RMTenthDry, fill = cum_sum))+
  ...
  facet_zoom(x = Date >= as.Date("2012-04-01") & Date < as.Date("2012-11-01"), zoom.size = 1)+
  theme_classic()
...
dat2.2 <- dat2 %>% 
  filter(RMTenthDry >= 74) %>% 
  mutate(RMTransformed = 170-RMTenthDry,
         KMTransformed = 273.5886-KMTenthDry)

jpeg("Figures/TileExtentWetDry.jpeg", units="in", width=6, height=8, res=300)

dat2.2 %>% 
  ggplot(aes(x = Date, y = KMTransformed, fill = as.factor(DryRM))) +
  geom_tile()+
  scale_fill_manual(name = "", values = c("gray94","#000000"), labels = c("dry", "wet"))+
  xlab("") + ylab("River kilometer")+
  scale_y_continuous(trans = "reverse")+
  #scale_x_date(date_breaks = "6 months", date_labels = "%Y-%m")+
  facet_zoom(x = Date >= as.Date("2012-04-01") & Date < as.Date("2012-10-31"), zoom.size = 1)+
  theme_classic()+
