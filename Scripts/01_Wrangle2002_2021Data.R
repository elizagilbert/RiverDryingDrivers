...
#Data 2020 ####
filter <- c("date", "text", "numeric", "numeric", "numeric", "numeric", 
...
dat2020_IsletaPer <- read_xlsx("Data/Raw/RiverEyes2020SummaryData.xlsx",
                               ...
                               range = cell_limits(c(1,1), c(NA,12)), col_types = filter) %>% 
...
#Data 2002-2021 ####
dat2002_2018 <- read.csv("Data/Raw/RiverEyesTenthMile2002_2018.csv") %>% 
  select(Year, X) %>% 
...
#bind 2019_2021
dat2002_2021 <- rbind(dat2019, dat2020, dat2021, dat2002_2018)

  #get all dates and tenths mile to join to data
DatesAll <- seq(as.Date("2002/1/1"), as.Date("2021/12/31"), "day")
RM_tenths <- seq(54,170, by = 0.1)
FillDatesRM <- as.data.frame(crossing(DatesAll, RM_tenths) %>% 
  rename(Date = DatesAll, RMTenthDry = RM_tenths))

  #join to get wet days
test1 <- dat2002_2021 %>% filter(Date > "2020/1/1")
test2 <- FillDatesRM %>% filter(between(Date, as.Date("2020/1/1"), as.Date("2020/12/31")))

dat2 <- FillDatesRM %>% 
  left_join(dat2002_2021, by = c("Date", "RMTenthDry")) %>% 
  mutate(Dry2 = as.factor(case_when(Dry == 0 ~0,
                          TRUE ~ 1))) %>% 
  select(Date, RMTenthDry, Dry2) %>% 
  rename(Date = 1, DryRM = 3)

write.csv(dat2, "Data/Processed/2002_2021_WetDryTenths.csv")
