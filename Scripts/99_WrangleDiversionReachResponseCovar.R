...
DistToDryR1 <- dat_drying %>% 
  select(!X) %>% 
  mutate(Date = as.Date(Date, format = "%Y-%m-%d")) %>% 
  filter(DryRM == 0 & RMTenthDry >116) %>% 
  group_by(Date) %>% 
  slice(which.max(RMTenthDry)) %>% 
  mutate(Discharge_DistToDry = 166-RMTenthDry) %>% 
  ungroup() %>% 
  complete(Date = seq.Date(as.Date("2010-01-01"), as.Date("2021-12-31"), by = "day")) %>% 
  mutate(Discharge_DistToDry = case_when(Discharge_DistToDry >-3 ~ Discharge_DistToDry,
                                         TRUE ~ 0)) %>% 
  select(Discharge_DistToDry)

#R1 combine all data frames 
Reach1 <- as.data.frame(cbind(Ext_ExtChng_R1, MileDays_R1, TempPrecip_R1, 
                              Discharge_R1, Diversions_R1, Returns_R1, DistToDryR1)) %>% 
  mutate(Reach = "R1")
...
Reach2 <- as.data.frame(cbind(Ext_ExtChng_R2, MileDays_R2, TempPrecip_R2, 
                              Discharge_R2, Diversions_R2, Returns_R2, DistToDryR2)) %>% 
  mutate(Reach = "R2")

#Final dataframe
DiversionSubreachData <- rbind(Reach1, Reach2) 
  
temp <- DiversionSubreachData %>% 
  filter(between(month(Date), 4, 10)) %>% 
  select(Date, Reach, Precip_mm, Temp_C, DiversionChng_cfs, Discharge_cfs, Returns_cfs, Discharge_DistToDry) %>% 
  pivot_longer(cols = Precip_mm:Discharge_DistToDry, names_to = "Predictor", values_to = "Values") %>% 
...
write.csv(temp, "Data/Processed/temp.csv", row.names = T)
